...
def get_and_prepare_data(start_date, end_date, stocks):
    ...
    for stock_id in stocks:
        try:
            ...
            # V15: 將 BBANDS 週期改為短線常用的 14 週期，並明確指定，以確保資料生成。
...
def run_simple_momentum_model(input_data):
    ...
    if not bb_upper_band:
        # 如果找不到 BBANDS 欄位，則將該分數設為 0，避免 KeyError
        df['Score_BB_Breakout'] = 0
    else:
        # ----------------------------------------------------------------
        # V15: 短線動能模型權重 (總分 1.00) 
        # ----------------------------------------------------------------
    ...
    # 如果找不到 BB 欄位，MACD, KDJ...等指標還是會計算。這裡需要一個總分的計算。
    if bb_upper_band is None:
        df['AI_Score'] = df['Score_MACD'] + df['Score_KDJ_Cross'] + df['Score_RSI_Strong'] + df['Score_Volume'] + df['Score_RSI_Oversold']
        df.loc[:, 'AI_Score'] = df['AI_Score'].fillna(0) # 確保分數欄位存在且無 NaN
