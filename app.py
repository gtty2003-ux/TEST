...
def main():
    ...
    # --- 產業篩選下拉選單 ---
    industry_options = list(INDUSTRY_MAP.keys())
    selected_industry = st.selectbox(
        '🔎 **選擇產業類別** (將篩選出該類別中股價 ≤ 50 元的 Top 5 動能股):',
        options=industry_options,
        index=0 # 預設選擇 '全部'
    )
    
    # 根據選擇獲取要分析的股票清單
    stocks_to_analyze = INDUSTRY_MAP.get(selected_industry, ALL_STOCKS_V18)
    stock_count = len(stocks_to_analyze)
    
    st.info(f'當前選股池涵蓋 **{selected_industry}**，共 **{stock_count} 檔** 股票。')
    ...
    with st.spinner('🔄 正在獲取數據並運行分析模型...'):
        processed_data = get_and_prepare_data(start_date.strftime('%Y-%m-%d'), end_date.strftime('%Y-%m-%d'), stocks_to_analyze)
    ...
    st.header(f'🏆 {selected_industry} 本日 Top 5 推薦清單')
    ...
    if final_recommendations.empty:
        st.warning(f'⚠️ 警告：在 {selected_industry} 產業，且股價 ≤ {PRICE_LIMIT} 元的範圍內，沒有股票符合任何動能信號。')
    else:
    ...
    markdown_notes = (
        ...
        f'- 產業清單是手動建立的精選代表股。選擇 **全部** 將分析約 1000 檔股票。'
    )
